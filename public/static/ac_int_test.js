  ...
  __g.misc.setApiVersionReceived(function() {
    ...
    if (spanVer) {
      ...
      if (this.apiVersion != this.sdkVersion) {
        ...
        logWithColor(
          "red",
          "<b>JS SDK版本和云渲染服务器版本不一致，可能造成接口调用错误，请确认!</b>\n"
        );
      } else {
...
function init(withPlayer, withInterface) {
  ...
  var spanVer = document.getElementById("spanVer");
  if (spanVer) spanVer.innerHTML = ` S300C${AcApiVersion}`;
  ...
  if (typeof CodeMirror != "undefined") {
    __editor = CodeMirror.fromTextArea(
...
function log(s, nnl) {
  ...
  e.innerHTML += s + (nnl ? "" : "\n");
  if (e.innerHTML.length > 1024 * 10 && !notAutoClear) e.innerHTML = "";
...
function showAllCommands() {
  ...
  var index = 0;
  var count = 0;
  for (var v in CommandType) count++;
    ...
    if (data.Type == "TileLayer") {
      ...
      __g.coord.pcs2gcs(data.MouseClickPoint, function(res) {
        if (res.coordinates.length > 0) {
          ...
          log(
            `鼠标点击位置：[${data.MouseClickPoint[0]}, ${data.MouseClickPoint[1]}, ${data.MouseClickPoint[2]}]`
          );
...
function test_camera_get() {
  __g.camera.get(function(res) {
    log(
      "这是camera.get的回调函数的输出信息，可以通过下面的代码重新设置到当前的位置：\n"
    );
...
function test_coord_gcs2pcs() {
  __g.coord.gcs2pcs([30.297492106590411, 113.9825982455081]);
}
...
function test_cameraTour_add() {
  ...
  frames.push(
    new CameraTourKeyFrame(
      0,
      1.0,
      [492501.90625, 2483838.75, 5898.237305],
      [-55.95829, -89.993935, 0]
    )
  );
  frames.push(
    new CameraTourKeyFrame(
      1,
      2.0,
      [493538.75, 2487061.5, 1166.874878],
      [-36.769756, -91.261223, 0]
    )
  );
  frames.push(
    new CameraTourKeyFrame(
      2,
      3.0,
      [493364.78125, 2487789.25, 504.430054],
      [-23.049517, -91.261223, 0]
    )
  );
  frames.push(
    new CameraTourKeyFrame(
      3,
      4.0,
      [495635.78125, 2491133.75, 183.135956],
      [-24.96583, -172.325165, 0]
    )
  );
  frames.push(
    new CameraTourKeyFrame(
      4,
      5.0,
      [495270, 2491256.75, 67.038582],
      [-25.314354, 108.269859, 0]
    )
  );
...
function test_tileLayer_show() {
  checkTileLayerId() && __g.tileLayer.show(__currentTileLayerActor.id);
}

function test_tileLayer_hide() {
  checkTileLayerId() && __g.tileLayer.hide(__currentTileLayerActor.id);
}
...
function test_tileLayer_disableXRay() {
  checkTileLayerId() && __g.tileLayer.disableXRay(__currentTileLayerActor.id);
}
...
function test_tileLayer_actor_show() {
  checkTileLayerId() &&
    __g.tileLayer.showActor(
      __currentTileLayerActor.id,
      __currentTileLayerActor.objectId
    );
}

function test_tileLayer_actor_hide() {
  checkTileLayerId() &&
    __g.tileLayer.hideActor(
      __currentTileLayerActor.id,
      __currentTileLayerActor.objectId
    );
}
...
function test_tileLayer_actor_showAllActors() {
  checkTileLayerId() && __g.tileLayer.showAllActors(__currentTileLayerActor.id);
}

function test_tileLayer_actor_hideAllActors() {
  checkTileLayerId() && __g.tileLayer.hideAllActors(__currentTileLayerActor.id);
}

function test_tileLayer_actor_enableClip() {
  checkTileLayerId() && __g.tileLayer.enableClip(__currentTileLayerActor.id);
}

function test_tileLayer_actor_disableClip() {
  checkTileLayerId() && __g.tileLayer.disableClip(__currentTileLayerActor.id);
}
...
async function test_tag_setCoordinate() {
  await __g.tag.setCoordinate("p1", [495254.1875, 2491154.25, 0.4]);
  __g.tag.focus("p1", 200, 0.2);
}
...
async function test_ctag_add() {
  ...
  let page =
    HostConfig.Path +
    "/int_custom_tag.html?icon=images/ctag1.png&title=北医三院&address=海淀区花园北路";
...
function test_ctag_update() {
  ...
  let page =
    HostConfig.Path +
    "/int_custom_tag.html?icon=images/ctag2.png&title=北京银行&address=朝阳区外馆斜街";
  let contentWeb = new WebUIData(page, 220, 52);
  let popupWeb = new WebUIData(HostConfig.Path + "/int_popup.html", 600, 480);
  let pivot = [0.5, 0.5];
  let range = [1, 5000];
...
function test_odline_add() {
  ...
  __g.odline.delete("od1").then(() => __g.odline.add(o));
}
...
function test_polygon_update() {
  let coords = [
    ...
    [
      [488248.65625, 2491142.25, 1],
      [488215.46875, 2491330.25, 1],
      [488057.71875, 2491184.25, 1],
    ],
  ];
...
function test_customObject_callFunction() {
  __g.customObject.callFunction(
    "o1",
    "funName",
    BPFuncParamType.Int32,
    10002,
    () => {}
  );
}
...
function test_panorama_add() {
  let o = new PanoramaData(
    "p1",
    HostConfig.Path + "/media/panorama1.jpg",
    [495302.625, 2491061.5, 30.15234375],
    75
  );
  __g.panorama.add(o);
}
...
function test_panorama_update() {
  let o = new PanoramaData(
    "p1",
    HostConfig.Path + "/media/panorama2.jpg",
    [495302.625, 2491061.5, 30.15234375],
    75
  );
...
function test_dynamicWater_add() {
  let coords = [
    [493878.9375, 2490438.25, 0],
    [494153.09375, 2490915, 0],
    [494413.875, 2490763.25, 0],
    [494286.21875, 2490366.25, 0],
  ];
...
function test_misc_addAnimatedImageButton() {
  ...
  let o = new AnimatedImageButtonData(
    1,
    x,
    y,
    width,
...
function test_misc_setMousePickMask() {
  ...
  let mask =
    MousePickMask.MouseClick |
    MousePickMask.MouseMove |
    MousePickMask.MouseHover;
...
function test_misc_playVideo() {
  __g.misc.playVideo(
    ...
    400,
    300,
    HostConfig.Path + "/media/video2.mov"
  );
}
...
function test_misc_lineIntersect() {
  __g.tools.lineIntersect(
    [1068.1212158203125, 4493.12060546875, 188.63694763183594],
    [1379.934326171875, 5068.63818359375, 6.7398433685302734]
  );
}
...
async function testcase_simulate_gps_location() {
  ...
  let index = 0;
  setInterval(async () => {
    if (++index > __gps_pos.length) index = 0;
...
function test_stress_polyline_show_hide_frequently() {
  ...
  __g.polyline.add(oa, function() {
    __g.polyline.focus(0, 0, () => {
      ...
      for (let i = 0; i < 100; i++) {
        for (let o of oa) {
          if (i % 2 == 0) __g.polyline.show(o.id);
          else __g.polyline.hide(o.id);
        }
      }
    });
  });
...
function test_stress_polygon_show_hide_frequently() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    let o = new PolygonData(
      i,
      color,
      zoneBoundary.geometries[i].coordinates[0],
      frameColor,
      frameThickness
    );
    oa.push(o);
  ...
  __g.polygon.add(oa, function() {
    __g.polygon.focus(0, 0, () => {
      ...
      for (let i = 0; i < 100; i++) {
        for (let o of oa) {
          if (i % 2 == 0) __g.polygon.show(o.id);
          else __g.polygon.hide(o.id);
        }
      }
    });
  });
...
function test_stress_3dpolygon_show_hide_frequently() {
  test_stress_3dpolygon_from_geojson(function(oa) {
    ...
    for (let i = 0; i < 100; i++) {
      for (let o of oa) {
        if (i % 2 == 0) __g.polygon3d.show(o.id);
        else __g.polygon3d.hide(o.id);
      }
    }
  });
}
...
function test_stress_3dpolygon_show_hide_frequently2() {
  ...
  test_polygon3d_add(function() {
    ...
    setInterval(() => {
      if (++i % 2) test_polygon3d_hide();
      else test_polygon3d_show();
    }, 500);
  });
}
...
function test_stress_playVideo_frequently() {
  ...
  setInterval(function() {
    for (let i = 0; i < 3; i++) {
      if (!isfirst) __g.misc.stopPlayVideo("test" + i);
